...
export default function UpdatePropertyDialog({isOpen, onClose, onSubmit, initialData, trigger,}: PropertyDialogProps) {
    const defaultPropertyValues: UpdatePropertyDTO = {
        id: initialData?.id,
    ...
    const form = useForm<UpdatePropertyDTO>({
        resolver: zodResolver(UpdatePropertySchema),
        defaultValues: defaultPropertyValues
    })

    const handleSubmit = (data: UpdatePropertyDTO) => {
        onSubmit(data);
        setTimeout(() => {
            onClose();
            form.reset();
            ...
            <DialogContent className="max-h-[80vh] overflow-y-auto">
                ...
                >
                    ...
                    <div className="flex flex-col gap-1">
                        <Label>Id</Label>
                        <Input hidden {...form.register("id")} placeholder="Id"/>
                        {form.formState.errors.id && (
                            <p className="text-red-400 text-sm">{form.formState.errors.id?.message}</p>
                        )}
                    </div>
